...
LOGIN_USERNAME = os.getenv("LOGIN_USERNAME_DRAKE", "").strip()
LOGIN_PASSWORD = os.getenv("LOGIN_PASSWORD_DRAKE", "").strip()
...
def upsert_submission(notion: Client, item: dict):
    ...
    if dt_gmt7:
        ...
        props["Thời gian nộp"] = {"date": {"start": dt_gmt7.isoformat()}}
